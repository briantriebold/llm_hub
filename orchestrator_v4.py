import json, time, datetime, argparse, re, requests, hashlib
...
STATE_F  = Path("state.json")  # tracks processed task IDs
...
def run_queue(path="tasks.jsonl"):
    state = load_state()
    processed = set(state.get("processed", []))
    p = Path(path)
    if not p.exists():
        print(f"[QUEUE] No tasks file found at {p.resolve()}")
        return
    count_in = 0
    count_run = 0
    with p.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                print(f"[QUEUE] Skip invalid JSON: {line[:120]}")
                continue
            goal = task.get("goal", "").strip()
            if not goal:
                print("[QUEUE] Skip task without 'goal'")
                continue
            tid = task.get("id") or task_id_for(goal)
            if tid in processed:
                print(f"[QUEUE] Skip already processed: {tid}")
                continue
            print(f"[QUEUE] Running task: {tid} -> {goal}")
            out_path, log_path = run_pipeline(goal)
            processed.add(tid)
            count_run += 1
    state["processed"] = sorted(processed)
    save_state(state)
    print(f"[QUEUE] Done. Total lines: {count_in}, processed this run: {count_run}, total processed: {len(processed)}")

# ---- CLI ----
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--goal", type=str, help="Run a single goal immediately.")
    parser.add_argument("--queue", action="store_true", help="Process tasks.jsonl queue.")
    args = parser.parse_args()

    if args.goal:
        run_pipeline(args.goal)
    elif args.queue:
        run_queue("tasks.jsonl")
    else:
        # default sample
        run_pipeline("Draft a 3-post intro for Triebold Institute (≤280 chars each) with a CTA to join the mailing list.")
